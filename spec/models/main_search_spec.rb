...
RSpec.describe MainSearch, type: :model do
  ...
  describe "#events" do
    context "search by src ip" do
      let!(:event1) { FactoryBot.create(:event, src_ip: "192.0.2.1") }
      let!(:event2) { FactoryBot.create(:event, src_ip: "198.51.100.1") }
    ...
    context "search by dst ip" do
      let!(:event1) { FactoryBot.create(:event, dst_ip: "192.0.2.1") }
      let!(:event2) { FactoryBot.create(:event, dst_ip: "198.51.100.1") }
    ...
    context "search sensor via :sensor" do
      let!(:event1) { FactoryBot.create(:event, sensor: 'sentinel') }
      let!(:event2) { FactoryBot.create(:event, sensor: 'quark') }
    ...
    context "search signature via :signature" do
      let!(:event1) { FactoryBot.create(:event, alert_signature_id: '11335577') }
      let!(:event2) { FactoryBot.create(:event, alert_signature_id: '22446688') }
    ...
    context "search http_hostname via :http_hostname" do
      let!(:event1) { FactoryBot.create(:event, http_hostname: '0.example.com') }
      let!(:event2) { FactoryBot.create(:event, http_hostname: '1.example.net') }
  ...
  describe "#signatures" do
    context "search by src ip" do
      let(:signature1) { FactoryBot.create(:signature, signature_id: 1111) }
      let(:signature2) { FactoryBot.create(:signature, signature_id: 2222) }
      let!(:event1) { FactoryBot.create_list(:event, 2, src_ip: "192.0.2.1", 
                                                 signature: signature1) }
      let!(:event2) { FactoryBot.create_list(:event, 2, src_ip: "198.51.100.1", 
